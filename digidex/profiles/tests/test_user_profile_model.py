...
def test_build_root_user_collection(new_user_profile):
    collection, created = new_user_profile.build_root_user_collection()
...
@pytest.mark.django_db
def test_build_user_collection_name(new_user_profile):
    expected_name = f"{new_user_profile.username}'s Collection"
    assert new_user_profile.build_user_collection_name() == expected_name, "Collection name should match the expected format."


@pytest.mark.django_db
def test_check_for_existing_collection(new_user_profile):
    users_root_collection, _ = new_user_profile.build_root_user_collection()
    # Create a new collection under 'Users'
    new_user_profile_collection = users_root_collection.add_child(name=f"{new_user_profile.username}'s Collection")
    # Check if the collection is detected correctly
    result = new_user_profile.check_for_existing_collection(f"{new_user_profile.username}'s Collection", users_root_collection)
    assert result == new_user_profile_collection, "Should find the existing collection correctly."


@pytest.mark.django_db(transaction=True)
def test_transaction_atomicity(new_user_profile, monkeypatch):
...
@pytest.mark.django_db
def test_prevent_multiple_user_collections(new_user_profile):
...
@pytest.mark.django_db
def test_user_collection_link(new_user_profile):
    # Create user collection and link it
    user_collection_link = new_user_profile.create_user_collection()

    # Verify that link is established correctly
    assert user_collection_link.user == new_user_profile
    collection_name = f"{new_user_profile.username}'s Collection"
