...
from apps.botany.models import GrowingLabel, Plant, PlantImage
...
class LabelModelTest(TestCase):
    def setUp(self):
        ...
        self.label1 = GrowingLabel.objects.create(name='Label 1', user=self.user)
        self.label2 = GrowingLabel.objects.create(name='Label 2', user=self.user)
        self.common_label = GrowingLabel.objects.create(name='Common Label', is_common=True)
    ...
    def test_label_unique_together(self):
        with self.assertRaises(Exception):
            GrowingLabel.objects.create(name='Label 1', user=self.user)

    def test_get_common_labels(self):
        common_labels = GrowingLabel.get_common_labels()
...
class PlantModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='testuser', password='testpass')
        self.label = GrowingLabel.objects.create(name='Test Label', user=self.user)
